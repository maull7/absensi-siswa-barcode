...
<div class="container">
    ...
    <h2 class="text-center">Data absen siswa pulang kelas <?=$class; ?></h2>
    <h4 class="text-center">Tanggal : <?= date("Y-m-d");?></h4>
...
document.querySelector('.send').addEventListener('click', function() {
    ...
    let today = new Date();
    let dayName = getNamaHari(today.getDay());  // Nama hari dalam bahasa Indonesia
    let day = today.getDate();                  // Tanggal
    let monthName = getNamaBulan(today.getMonth()); // Nama bulan dalam bahasa Indonesia
    let year = today.getFullYear();             
    let tanggal = `${dayName}, ${day} ${monthName} ${year}`;

    // Menyiapkan teks judul
    let titleText = `Report Harian pulang Siswa kelas ${kelas}`;
    ...
    doc.autoTable({
        ...
        head: [['No', 'NIS', 'Nama', 'Kelas', 'Tanggal', 'Jam Pulang Siswa']],
        body: data
    });
